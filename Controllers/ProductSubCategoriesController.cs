    ...
    public class ProductSubCategoriesController : Controller
    {
        ...
        public async Task<ActionResult> Create(ProductSubcategoryBL ProSubCategory)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                //Sending request to find web api REST service resource GetAllEmployees using HttpClient
                HttpResponseMessage Res = await client.PostAsync("https://localhost:44390/api/ProductSubCategories/", content);
                //Checking the response is successful or not which is sent using HttpClient
                if (Res.IsSuccessStatusCode)
                {
                    //Storing the response details recieved from web api
        ...
        public async Task<ActionResult> Edit(int ProductSubCategoryId)
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    HttpResponseMessage Res = await client.GetAsync("https://localhost:44390/api/ProductSubCategories/" + ProductSubCategoryId);
                    if (Res.IsSuccessStatusCode)
                    {
                        //Storing the response details recieved from web api
                        var ProResponse = Res.Content.ReadAsStringAsync().Result;
                    ...
                    HttpResponseMessage Res2 = await client.GetAsync("https://localhost:44390/api/ProductCategories/");
