    ...
    public class CustomerController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage Res = await client.GetAsync("https://localhost:44390/api/Customer/");
        ...
        public async Task<ActionResult> Create(CustomerBL customer)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                //Sending request to find web api REST service resource GetAllEmployees using HttpClient
                HttpResponseMessage Res = await client.PostAsync("https://localhost:44390/api/Customer/", content);
                //Checking the response is successful or not which is sent using HttpClient
                if (Res.IsSuccessStatusCode)
                {
                    //Storing the response details recieved from web api
        ...
        public async Task<ActionResult> Edit(int CustomerId)
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    HttpResponseMessage Res = await client.GetAsync("https://localhost:44390/api/Customer/" + CustomerId);
                    if (Res.IsSuccessStatusCode)
                    {
                        //Storing the response details recieved from web api
                        var ProResponse = Res.Content.ReadAsStringAsync().Result;
                    ...
                    using (var package = new ExcelPackage(file.InputStream))
                    {
                        ...
                        for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                        {
                            if (workSheet.Cells[rowIterator, 1].Value != null)
                            {
                                ...
                                customerBL.CreateDate = Convert.ToDateTime(workSheet.Cells[rowIterator, 7].Value);
